...
class Door:
	...
	func to_str():
		...
		if _type == DoorType.DOOR_ARC:
			s += "%d %.2f°->%.2f°: %s::%s" % [
				l, rad_to_deg(t0), rad_to_deg(t1),
				room1.to_str(), room2.to_str(),
			]
...
class Shape extends MazeShape:
	...
	var min_theta: float:
		get(): return resource.min_theta
	var max_theta: float:
		get(): return resource.max_theta
	var min_level: int:
		get(): return resource.min_level
	var max_level: int:
		get(): return resource.max_level
	## The actual length of `r` for each level
	var level_width: float:
		get(): return resource.level_width
	var lowest_level_room_theta: float:
		get(): return resource.lowest_level_room_theta
	var room_justify: MazeCircleResource.CircleJustify:
		get(): return resource.room_justify
	...
	func get_rooms(t0: float, t1: float, l: int):
		...
		return rooms_by_l(l).filter(func(room: Room):
			return (
